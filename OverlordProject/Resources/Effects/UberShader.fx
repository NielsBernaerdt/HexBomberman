...
{	
	...
	normalSample = (2.0f * normalSample) - float4(1.0f, 1.0f, 1.0f, 1.0f);
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
    ...
    if(gUseEnvironmentMapping)
    {
    	float3 refractVec = refract(normal, viewDirection, gRefractionIndex);
    	float3 environmentSampleRefract = gCubeEnvironment.Sample(gTextureSampler,refractVec);
    	float3 reflectVec = reflect(normal,viewDirection);
