...
#pragma region POST-PROCESSING PASS
	...
	if (m_PostProcessingMaterials.size() > 0)
	{
		...
		for (const auto postProcessingMaterial : m_PostProcessingMaterials)
		{
			if (!postProcessingMaterial->IsEnabled())
			{
				continue;
			}
			postProcessingMaterial->Draw(m_SceneContext, PREV_RT);
			PREV_RT = postProcessingMaterial->GetOutput();
		}
		if (PREV_RT != INIT_RT)
		{
